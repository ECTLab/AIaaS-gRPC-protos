...
PYTHON_PROTOS_GENERATOR_SCRIPT=$(SCRIPTS_DIR)/generate_python_protos.sh
...
PYTHON_STUBS_OUTPUT_FILE_PATH=$(PYTHON_STUBS_OUTPUT_DIR)/stubs.py
PYTHON_STUBS_TEMPLATE_OUTPUT_FILE_PATH=$(TEXT_FILES_DIR)/python_stubs_py_template.txt
...
generate_python_protos: clean install_python_requirements
	$(PYTHON_PROTOS_GENERATOR_SCRIPT) $(PROTOS_DIR) $(PYTHON_PROTOS_OUTPUT_DIR)
...
generate_python_stubs: generate_python_protos
	python \
		...
		$(PYTHON_STUBS_OUTPUT_FILE_PATH) \
		$(PYTHON_STUBS_TEMPLATE_OUTPUT_FILE_PATH)
